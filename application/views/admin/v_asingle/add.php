			...
			<div class="body">
				<form id="form_validation" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group form-float">
						...
						<div class="form-line">
							<input type="text" class="date-full form-control" name="release_date" required autocomplete="off">
						</div>
					</div>
					...
					<div class="form-group form-float">
						...
						<div class="form-line">
							<input type="text" class="date-own form-control" name="year_production" required autocomplete="off">
						</div>
					</div>
...
$('#title').change(function () {
	$.ajax({
		type: "POST",
		url: "<?php echo base_url('Admin_single/checkTitle');?>",
		data: {
			user_id: $('#user_id').val(),
			title: $('#title').val()
		},
...
window.onload = () => {
  ...
  var uploader = new plupload.Uploader({
    runtimes: "html5",
    browse_button: "pick",
    url: "<?php echo base_url('Admin_single/test'); ?>",
    ...
    init: {
      ...
      PostInit: () => list.innerHTML = "<div id='ready>Ready</div>",
      FilesAdded: (up, files) => {
		var allowedTypes = ["audio/wav", "audio/x-wav", "audio/wave", "audio/x-pn-wav"];
		var title = $('#title').val();
  ...
  // Tambahkan event listener untuk menangkap perubahan
  userDropdown.addEventListener("change", function () {
    uploader.settings.multipart_params.user_id = this.value;
  });
...
$('#form_validation').submit(function(e) {
	...
	if (title_copy == 'exists') {
		...
		$.ajax({
				...
				url: '<?php echo base_url('Admin_single/prosesAdd'); ?>',
				type: "post",
...
function save_berhasil() {
    Swal.fire({
        ...
        var link = '<?php echo base_url("/v2/user/single") ?>';
...
$(document).on("click", ".back", function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                method: "POST",
                url: "<?php echo base_url('Admin_single/cancelUpload'); ?>",
                data: {
					"order_id": order_id,	
					"file_name": file_name
				},
